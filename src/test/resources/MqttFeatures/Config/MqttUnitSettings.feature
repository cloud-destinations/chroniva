...
@MqttUnitsettings3 
  Scenario: Validate Morning Cool Down in VAV Control selection 
      When user loads data from Excel sheet "Unit Settings" row 5, publishes the MQTT message, and verifies the radio button name in row 5   
            Then User clicks next page button of the selected option >  
            Then the user navigates to "Config" 📊
...
@MqttUnitsettings4 
  Scenario: Validate Enable Unit Heating in VAV Control selection 
      When user loads data from Excel sheet "Unit Settings" row 5, publishes the MQTT message, and verifies the radio button name in row 5   
            Then User clicks next page button of the selected option >  
            Then the user navigates to "Config" 📊
   When user loads data from Excel sheet "Unit Settings" row 38, publishes the MQTT message, and verifies the radio button name in row 38   
           Then User clicks next page button of the selected option >  
             ...
             Then the user navigates to "Duct Static SP" 📊
             When User publishes random payload from Excel sheet "Unit Settings" row 47 and verifies UI Label
             Then User clicks back button
 
               Then the user navigates to "SAT Reset" 📊
     When user loads data from Excel sheet "Unit Settings" row 48, publishes the MQTT message, and verifies the radio button name in row 38   
     When user loads data from Excel sheet "Unit Settings" row 49, publishes the MQTT message, and verifies the radio button name in row 38   
